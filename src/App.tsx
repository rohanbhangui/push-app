...
const App = () => {
  ...
  return (
    <main className="bg-black flex flex-col justify-center items-center h-full min-h-screen">
      <Timer activeTimer={activeTimer !== null ? activeTimer.id : ''} timerTitle={activeTimer !== null ? activeTimer.title : ''} startTime={activeTimer !== null ? activeTimer.seconds : 900} />
      ...
      <ul className="mt-4 w-full max-w-sm flex flex-col space-y-2">
        {timers.map((timer) => (
            ...
            <div>
            <span>{Math.floor(timer.seconds / 60)}m {timer.seconds % 60}s</span>
            <button 
              onClick={(e) => {
              e.stopPropagation();
